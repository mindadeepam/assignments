...
for sent1, others in train_data.items():
    if len(others['entailment']) > 0 and len(others['contradiction']) > 0:
        ...
        # debug_samples.append([sent1, random.choice(list(others['entailment'])), random.choice(list(others['contradiction']))] )
        # debug_samples.append([random.choice(list(others['entailment'])), sent1, random.choice(list(others['contradiction']))] )

logging.info("Train samples: {}".format(len(train_samples)))

print("++++++++++++++++++++++++++++++++++")
# print(debug_samples[0], debug_samples[1])
print("here")
quit()
...
dev_evaluator = EmbeddingSimilarityEvaluator.from_input_examples(dev_samples, batch_size=train_batch_size, name='sts-dev', write_csv=True)
